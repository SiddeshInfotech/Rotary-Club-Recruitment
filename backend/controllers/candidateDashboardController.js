...
if (process.env.RAZORPAY_KEY_ID && process.env.RAZORPAY_KEY_SECRET) {
  ...
  console.warn("Razorpay API keys missing in environment variables. Payment features will be disabled.");
}
...
exports.createOrder = async (req, res) => {
  try {
    ...
    if (!razorpay) {
      // Fallback to Developer Mock Mode if keys are not present
      const mockOrderId = `order_mock_${Math.random().toString(36).substring(2, 10)}${Date.now()}`;
      return res.json({
        success: true,
        orderId: mockOrderId,
        amount,
...
exports.verifyPayment = async (req, res) => {
  try {
    ...
    // Handle verification for mock payments
    if (razorpay_order_id.startsWith("order_mock_") && razorpay_payment_id.startsWith("pay_mock_")) {
      ...
      return res.json({
        success: true,
        message: "[Mock Mode] Simulated payment verified successfully.",
        data: {
          isPremium: true,
    ...
    if (!process.env.RAZORPAY_KEY_SECRET) {
      return res.status(500).json({ success: false, message: "Razorpay keys are not configured on this server." });
    }
