...
export default function EQProfileCard({ eqScores }) {
    ...
    const labelPositions = [
        { className: "absolute -top-4 left-1/2 -translate-x-1/2 text-center", key: 'leadership' },
        { className: "absolute top-10 right-0", key: 'loyalty' },
        { className: "absolute top-1/2 -right-4 -translate-y-1/2", key: 'adaptability' },
        { className: "absolute bottom-10 right-0 max-w-[60px] text-center leading-tight", key: 'growthMindset' },
        { className: "absolute -bottom-4 left-1/2 -translate-x-1/2 text-center", key: 'reliability' },
        { className: "absolute bottom-10 left-2", key: 'teamwork' },
        { className: "absolute top-1/2 -left-6 -translate-y-1/2", key: 'collaboration' },
        { className: "absolute top-10 left-2 max-w-[60px] text-center leading-tight", key: 'problemSolving' },
    ];
        ...
        <div className="bg-white dark:bg-[#131b2f] border border-slate-200 dark:border-[#1e293b] rounded-2xl p-10 flex flex-col items-center justify-between shadow-sm dark:shadow-none hover:shadow-md transition relative overflow-hidden">
            ...
            <div className="flex-1 w-full flex items-center justify-center relative my-6">
                ...
                {labelPositions.map((lp) => (
                    <span
                        key={lp.key}
                        className={`${lp.className} text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest bg-white dark:bg-[#131b2f] px-1.5 py-0.5 rounded`}
                    >
                        {traitLabels[lp.key]}
                    </span>
                ))}
