...
import { ArrowLeft, Mail, Lock, Loader, Eye, EyeOff } from 'lucide-react';
...
export default function Login() {
    ...
    const savedCreds = JSON.parse(localStorage.getItem('eqhire_remembered_creds') || 'null');
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (res.data.success) {
                // Save JWT token
                if (rememberMe) {
                    localStorage.setItem('eqhire_token', res.data.token);
                    // Remember credentials for next visit
                    localStorage.setItem('eqhire_remembered_creds', JSON.stringify({ email, password }));
                } else {
                    sessionStorage.setItem('eqhire_token', res.data.token);
                    localStorage.removeItem('eqhire_remembered_creds');
                }

                // Save user to context (AuthContext handles storage based on rememberMe)
                login(res.data.user, rememberMe);
                navigate(res.data.user.role === 'recruiter' ? '/recruiter' : '/candidate');
            }
        } catch (err) {
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-[#0b1121] flex items-center justify-center p-6 transition-colors duration-300 relative overflow-hidden">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 md:p-12 max-w-[480px] w-full shadow-xl shadow-slate-200/50 dark:shadow-none relative z-10">
                ...
                {error && (
                    <div className="mb-6 px-4 py-3 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 text-red-600 dark:text-red-400 text-sm font-semibold">
                        {error}
                    </div>
                )}
                ...
                <form className="space-y-6" onSubmit={handleLogin}>
                    ...
                    <div className="pt-4">
                        <button
                            type="submit"
                            disabled={isLoading}
                            className="w-full bg-blue-600 hover:bg-blue-500 dark:bg-white dark:text-slate-900 text-white font-bold py-4 rounded-xl transition-all shadow-lg shadow-blue-500/20 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                        >
                            {isLoading ? (
                                <>
