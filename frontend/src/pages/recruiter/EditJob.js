...
export default function EditJob() {
    ...
    useEffect(() => {
        ...
        const fetchJob = async () => {
            try {
                // jobMatchController exposes GET /api/jobs/job/:id
                ...
                if (res.data.success) {
                    ...
                    setForm({
                        ...
                        qualification: job.education?.qualification || 'Graduate',
                        allowBacklogs: job.education?.allowBacklogs || false,
        ...
        return (
            <RecruiterLayout>
                <div className="flex flex-col items-center justify-center py-24">
                    ...
                    <h2 className="text-3xl font-black text-slate-900 dark:text-white mb-2 tracking-tighter">Job Updated!</h2>
    ...
    return (
        <RecruiterLayout>
            <div className="max-w-3xl mx-auto space-y-8 pb-10">
                <div>
                    <h1 className="text-2xl font-bold text-[#1a2b4b] dark:text-white font-serif tracking-tight">Edit Job Listing</h1>
                ...
                <form onSubmit={handleSubmit} className="bg-white dark:bg-[#131b2f] rounded-xl shadow-sm border border-gray-200 dark:border-slate-800 p-6 space-y-6">
                    ...
                    <div className="flex justify-end gap-3 pt-4">
                        <button type="button" onClick={() => navigate('/recruiter/jobs')} className="px-6 py-2 text-sm font-bold text-slate-500">Cancel</button>
                        <button type="submit" disabled={loading} className="px-8 py-2 bg-blue-600 text-white rounded-lg text-sm font-bold hover:bg-blue-700 flex items-center gap-2 transition-all active:scale-[0.98]">
                            {loading ? 'Saving...' : 'Save Changes'} <ArrowRight className="w-4 h-4" />
                        </button>
                    </div>
                </form>
            </div>
