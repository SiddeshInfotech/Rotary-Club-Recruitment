            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden h-[calc(100vh-140px)] flex">
                ...
                <div className="w-1/3 border-r border-gray-100 flex flex-col">
                    <div className="p-4 border-b border-gray-100 flex-shrink-0">
                        <h2 className="text-lg font-bold text-[#1a2b4b]">Messages</h2>
                        <div className="relative mt-3">
                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-4 h-4" />
                            <input 
                                type="text" 
                                placeholder="Search messages..." 
                                className="w-full pl-9 pr-4 py-2 rounded-lg bg-gray-50 border border-gray-100 focus:outline-none focus:ring-2 focus:ring-cyan-500/40 text-sm"
                            />
                        </div>
                    </div>
                    <div className="overflow-y-auto flex-grow">
                        {contacts.map(c => (
                            <div key={c.id} className={`p-4 border-b border-gray-50 cursor-pointer hover:bg-gray-50 transition-colors flex gap-3 ${c.active ? "bg-cyan-50/30" : ""}`}>
                                ...
                                <div className="flex-grow min-w-0">
                                    <div className="flex justify-between items-baseline mb-0.5">
                                        <h3 className="text-sm font-bold text-[#1a2b4b] truncate">{c.name}</h3>
                ...
                {/* Chat Area */}
                <div className="flex-grow flex flex-col bg-[#fcfdfd]">
                    {/* Header */}
                    <div className="h-16 border-b border-gray-100 flex items-center justify-between px-6 flex-shrink-0 bg-white">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="text-[15px] font-bold text-[#1a2b4b] leading-tight">{contacts[0].name}</h3>
                    ...
                    <div className="flex-grow overflow-y-auto p-6 space-y-6">
                        <div className="flex justify-center">
                            <span className="text-[11px] font-semibold text-gray-400 bg-gray-50 px-3 py-1 rounded-full border border-gray-100">Today</span>
                        </div>
                        
                        <div className="flex gap-3">
                            <img src={contacts[0].avatar} alt={contacts[0].name} className="w-8 h-8 rounded-full flex-shrink-0 mt-1" />
                            <div className="flex flex-col gap-1 max-w-[70%]">
                                <div className="bg-white border border-gray-100 px-4 py-2.5 rounded-2xl rounded-tl-none shadow-sm text-sm text-gray-700">
                    ...
                    <div className="p-4 bg-white border-t border-gray-100 flex-shrink-0">
                        <div className="flex gap-3 items-center bg-gray-50 border border-gray-200 rounded-full pl-4 pr-1 py-1 focus-within:ring-2 focus-within:ring-cyan-500/40 focus-within:border-cyan-500 transition-all">
                            ...
                            <input 
                                ...
                                className="flex-grow bg-transparent border-none focus:outline-none focus:ring-0 text-sm py-2"
                            />
